...
<style>
    ...
    .card-stat-elegant:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.1) !important; }
...
<div class="container py-5 mt-2 animate__animated animate__fadeIn">
    ...
    <div class="page-header-elegant d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="fw-bold mb-0 text-dark" style="border-left: 5px solid #000; padding-left: 15px;">Ringkasan Statistik</h2>
        ...
        <div class="table-responsive">
            <table class="table table-elegant hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            @php
                                ...
                                $currentStatusClass = $statusClass[$item->status] ?? $statusClass['Menunggu Verifikasi Admin'];
                            @endphp
                            <span class="badge badge-pill-custom {{ $currentStatusClass }} bg-opacity-10 border border-opacity-25">
                    ...
                    @empty
                    <tr><td colspan="5" class="text-center py-5 text-muted italic small">Belum ada aktivitas pesanan terbaru.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="modal-content modal-content-elegant shadow-lg">
            ...
            <form id="formUpdatePesanan" method="POST">
                ...
                <div class="modal-body p-4 bg-white">
                    <div class="row mb-4">
                        <div class="col-6">
                            <label class="text-muted small fw-bold text-uppercase d-block mb-1">Identitas Pembeli</label>
                    ...
                    <div id="md-alamat-section" class="mb-3 p-3 bg-info bg-opacity-10 border border-info border-opacity-25 rounded-4" style="display: none;">
                        <label class="text-info small fw-bold text-uppercase d-block mb-1"><i class="fas fa-truck me-1"></i> Alamat Pengiriman Bus:</label>
                    ...
                    <div class="p-3 bg-light rounded-4">
                        ...
                        <div class="info-price-box mb-3 shadow-sm bg-white">
                            ...
                            <div id="display_ongkir_row" class="d-flex justify-content-between">
                                <span class="small text-muted">Ongkos Kirim Bus:</span>
                        ...
                        <div class="mt-3">
                            <label class="form-label small fw-bold text-uppercase"><i class="fas fa-tasks me-1"></i> Update Status Pesanan</label>
                            <select name="status" id="select_status" class="form-select border-dark shadow-sm rounded-3">
                ...
                <div class="modal-footer border-0 p-4 pt-0" id="modal-footer-admin">
                    <button type="button" class="btn btn-light rounded-pill px-4 fw-bold text-muted" data-bs-dismiss="modal">Tutup</button>
                    <button type="submit" class="btn btn-dark rounded-pill px-4 shadow-sm fw-bold">Update Status</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function showAdminDetail(data) {
        ...
        if (data.gambar_referensi) {
            gambarContainer.innerHTML = `<a href="/storage/${data.gambar_referensi}" target="_blank"><img src="/storage/${data.gambar_referensi}" class="img-fluid rounded-3" style="max-height: 200px; cursor: zoom-in;"></a>`;
        } else {
        ...
        if (data.status === 'Diverifikasi' || data.status === 'Ditolak' || data.status === 'Menunggu Verifikasi Admin') {
