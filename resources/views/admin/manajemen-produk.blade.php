    ...
    <div class="container py-5 mt-2 animate__animated animate__fadeIn">
        ...
        <div class="page-header-elegant d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h2 class="fw-bold mb-0 text-dark" style="font-family: 'Playfair Display', serif;">Manajemen Master Bahan
                    </h2>
                    <p class="text-muted small mb-0">Kelola daftar material dasar marmer untuk standarisasi katalog</p>
                </div>
            </div>
            <button type="button" class="btn btn-gold px-4 py-2 rounded-pill shadow-sm fw-bold" data-bs-toggle="modal"
                            ...
                            <tr>
                                ...
                                <td class="text-center pe-5">
                                    <div class="d-flex justify-content-center gap-3">
                                        {{-- Tombol Edit --}}
                                        <button class="btn btn-link text-primary p-0 fs-5" title="Edit"
                                            onclick="editBahan({{ json_encode($b) }})">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        {{-- Tombol Hapus dengan SweetAlert --}}
                                        <form id="form-delete-{{ $b->id }}"
                                            action="{{ route('admin.bahan.hapus', $b->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="button" class="btn btn-link text-danger p-0 fs-5" title="Hapus"
                                                onclick="confirmDelete({{ $b->id }}, '{{ $b->nama_bahan }}')">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modal-content-elegant shadow-lg">
                <div class="modal-header modal-header-elegant">
                    <h5 class="modal-title fw-bold" id="modalTitle"><i class="fas fa-edit me-2 text-gold"></i> Form Master
                        Bahan</h5>
                ...
                <div class="modal-body p-5 bg-white">
                    <form id="formBahan" action="{{ route('admin.bahan.simpan') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-light border-end-0 rounded-start-4"><i
                                        class="fas fa-gem text-gold"></i></span>
                        ...
                        <div class="d-grid gap-2 mt-5">
                            ...
                            <button type="button" class="btn btn-light py-2 fw-bold text-muted border-0"
    ...
    <script>
        ...
        function confirmDelete(id, nama) {
            Swal.fire({
                ...
                html: `
            Apakah Anda yakin ingin menghapus bahan <b>${nama}</b>?<br>
            <small class="text-danger">
                Data yang dihapus tidak dapat dikembalikan.<br>
                Produk di katalog yang menggunakan bahan ini juga akan ikut terhapus.
            </small>
        `,
                icon: 'warning',
