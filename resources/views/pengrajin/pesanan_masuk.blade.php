...
<style>
    ...
    body { background-color: var(--adira-cream); }
    .page-header-elegant { background: white; padding: 2rem; border-radius: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 2rem; border-left: 5px solid var(--adira-gold); }
    ...
    .detail-card { border: none; border-radius: 24px; background: white; box-shadow: 0 15px 40px rgba(0,0,0,0.08); position: sticky; top: 100px; border: 1px solid rgba(197, 164, 126, 0.2); }
    .info-group { background: #fcfbf7; padding: 1rem; border-radius: 15px; margin-bottom: 1rem; border: 1px solid #f1ece1; }
    .info-label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1px; color: #999; font-weight: 700; margin-bottom: 5px; display: block; }
    .info-value { font-weight: 700; color: var(--adira-dark); margin-bottom: 0; }
    .catatan-box { background: white; border-left: 4px solid var(--adira-gold); padding: 1rem; border-radius: 8px; font-style: italic; color: #555; min-height: 60px; }
    .gambar-referensi-wrapper { background: #f8f9fa; border-radius: 15px; overflow: hidden; border: 1px dashed #ddd; text-align: center; margin-bottom: 1rem; position: relative; min-height: 150px; display: flex; align-items: center; justify-content: center; }
    .img-referensi { max-width: 100%; max-height: 250px; object-fit: contain; cursor: pointer; transition: 0.3s; }
    .btn-konfirmasi { background-color: var(--adira-gold); color: white; font-weight: 800; border-radius: 50px; padding: 15px; border: none; transition: 0.3s; width: 100%; text-transform: uppercase; }
    .btn-konfirmasi:hover { background-color: #b08d44; transform: translateY(-3px); box-shadow: 0 5px 15px rgba(197, 164, 126, 0.4); }
</style>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="pe-4 text-center">
                                    <button class="btn btn-outline-dark btn-sm rounded-pill px-3 fw-bold btn-show-detail" 
...
document.querySelectorAll('.btn-show-detail').forEach(button => {
    button.addEventListener('click', function() {
        ...
        document.getElementById('detail-catatan').innerText = (catatan && catatan.trim() !== "") ? `"${catatan}"` : '"Tidak ada catatan kustomisasi."';
