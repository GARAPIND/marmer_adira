...
<style>
    ...
    .card-stat-elegant:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.05) !important; }
    ...
    .btn-gold:hover { background-color: #b08d44; color: white; transform: scale(1.05); }
            ...
            <div class="card card-stat-elegant p-4 shadow-sm border-start border-4 border-warning">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <h2 class="fw-bold m-0 text-dark">{{ $pesanan->where('status', 'Menunggu Verifikasi Admin')->count() }}</h2>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row g-4">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if($item->status == 'Menunggu Verifikasi Admin')
                                        <span class="badge badge-status-custom bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">Verifikasi Admin</span>
                                    @elseif($item->status == 'Diproses')
                                        <span class="badge badge-status-custom bg-info bg-opacity-10 text-info border border-info border-opacity-25">Sedang Diproses</span>
                                    @else
                                        <span class="badge badge-status-custom bg-success bg-opacity-10 text-success border border-success border-opacity-25">{{ $item->status }}</span>
                                    @endif
                                </td>
                                <td class="text-end pe-4">
                                    <a href="{{ route('pesanan.index') }}" class="btn btn-outline-dark btn-sm rounded-pill px-3 fw-bold">Pantau</a>
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="4" class="text-center py-5 text-muted small italic">Belum ada riwayat pesanan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="d-grid gap-3">
                <a href="{{ route('pesanan.create') }}" class="card p-3 quick-link-card shadow-sm">
                    <div class="d-flex align-items-center">
                        <div class="marble-icon-box me-3" style="width: 45px; height: 45px; font-size: 1.2rem;">
                            <i class="fas fa-pen-nib"></i>
                        </div>
                        <div>
                            <span class="fw-bold d-block text-dark">Pesan Custom</span>
                            <small class="text-muted">Buat desain marmer impian Anda</small>
                        </div>
                    </div>
                </a>
            ...
            <div class="card mt-4 border-0 rounded-4 shadow-lg" style="background: var(--adira-dark);">
                <div class="card-body p-4 text-center">
                    ...
                    <a href="https://wa.me/your-number" class="btn btn-gold w-100 py-2 shadow-sm">Chat Admin Sekarang</a>
                </div>
            </div>
        </div>
    </div>
