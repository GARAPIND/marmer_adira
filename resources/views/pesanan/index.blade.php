...
<div class="container py-5 mt-2 animate__animated animate__fadeIn">
    ...
    <div class="page-header-elegant d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="fw-bold mb-0 text-dark" style="border-left: 5px solid #000; padding-left: 15px;">Riwayat Pesanan</h2>
                            ...
                            <tr id="row-{{ $item->id }}">
                                ...
                                <td class="text-center">
                                    @if($item->status == 'Selesai')
                                        <span class="badge badge-status-pill bg-success bg-opacity-10 text-success border border-success border-opacity-25"><i class="fas fa-check-circle me-1"></i> {{ $item->status }}</span>
                                    @elseif($item->status == 'diekspedisi')
                                        <span class="badge badge-status-pill bg-info bg-opacity-10 text-info border border-info border-opacity-25"><i class="fas fa-bus me-1"></i> Dikirim</span>
                                    @elseif($item->status == 'Ditolak')
                                        <span class="badge badge-status-pill bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25">{{ $item->status }}</span>
                                    @else
                                        <span class="badge badge-status-pill bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25">{{ $item->status }}</span>
                                    @endif
                                </td>
                                <td class="text-end pe-4">
                                    <button class="btn btn-outline-dark btn-sm rounded-pill px-3 fw-bold" onclick="showDetail({{ json_encode($item) }})">Detail</button>
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="5" class="text-center py-4 text-muted italic small">Belum ada riwayat transaksi.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="table table-elegant hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr id="row-{{ $item->id }}">
                                ...
                                <td class="fw-semibold text-dark">{{ $item->nama_produk }}</td>
                                <td class="text-center">
                                    <span class="badge badge-status-pill bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">Menunggu</span>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-2">
                                        <button class="btn btn-dark btn-sm rounded-pill px-3 fw-bold" onclick="showDetail({{ json_encode($item) }})">Lihat</button>
                                        {{-- Tombol Batal Langsung di Tabel --}}
                                        <button class="btn btn-outline-danger btn-sm rounded-pill px-3 fw-bold" onclick="konfirmasiBatal({{ $item->id }}, '{{ $item->nama_produk }}')">
                            ...
                            @empty
                            <tr><td colspan="5" class="text-center py-4 text-muted small italic">Tidak ada pesanan yang sedang diverifikasi.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card card-detail-sidebar animate__animated animate__fadeInRight">
                <div class="card-body p-4">
                    <div id="empty-state" class="text-center py-5">
                        ...
                        <p class="text-muted small px-3">Pilih salah satu pesanan untuk melihat rincian lengkap & instruksi pembayaran.</p>
                    </div>

                    <div id="detail-content" style="display: none;">
                        <h5 class="fw-bold border-bottom pb-2 mb-4 text-dark">Detail Pesanan <span id="det-id" class="text-gold"></span></h5>
                        
                        <div class="mb-4">
                            <label class="text-muted small fw-bold text-uppercase d-block mb-1">Produk & Material</label>
                        ...
                        <div class="mb-4 p-3 price-breakdown-box shadow-sm">
                            <label class="text-gold small fw-bold text-uppercase d-block mb-2"><i class="fas fa-receipt me-1"></i> Rincian Pembayaran</label>
                        ...
                        <div class="alert alert-secondary border-0 py-2 small d-flex align-items-center rounded-3">
                            ...
                            <div>
                                ...
                                <div id="det-alamat-full" class="fw-bold text-dark" style="font-size: 0.75rem;"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    function showDetail(data) {
        ...
        document.getElementById('det-metode').innerText = data.metode_pengambilan === 'dikirim' ? 'Dikirim (Via Bus)' : 'Ambil Dirumah';
        ...
        if(data.status == 'Menunggu Verifikasi Admin') {
            ...
            if(data.status === 'Ditolak') {
                labelStatus.innerHTML = '<span class="badge w-100 bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 py-2"><i class="fas fa-times-circle me-1"></i> Pesanan Ditolak</span>';
            } 
            else if(data.status === 'Selesai') {
                labelStatus.innerHTML = '<span class="badge w-100 bg-success bg-opacity-10 text-success border border-success border-opacity-25 py-2"><i class="fas fa-check-circle me-1"></i> Pesanan Selesai</span>';
            }
                ...
                labelStatus.innerHTML = '<span class="badge w-100 bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 py-2"><i class="fas fa-tools me-1"></i> ' + data.status + '</span>';
            }
        }
